...
def plot_data_vs_predictions(x_train, y_train, x_test, y_test, y_all_pred, grid, g_star_preds, fn_y, args, plot_type):
    fig = go.Figure()
    ...
    wandb.log({f"plot_{plot_type}": fig})
    ...
    fig.write_image(f"plots/fit/{args.dataset}_{args.generalisation_task}_n={args.hidden_units}_"
                    f"{normalised}_{adjusted}_{plot_type}.png")
